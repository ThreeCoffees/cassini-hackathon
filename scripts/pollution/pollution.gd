...
func propagate():
	var rand = rng.randf()
	if rand < 0.25:
		if not pollution_manager.is_polluted(Vector2(position.x+24,position.y)):
			pollution_manager.add_polution(Vector2(position.x+24,position.y))
	elif rand < 0.5:
		if not pollution_manager.is_polluted(Vector2(position.x-24,position.y)):
			pollution_manager.add_polution(Vector2(position.x-24,position.y))
	elif rand < 0.75:
		if not pollution_manager.is_polluted(Vector2(position.x,position.y+24)):
			pollution_manager.add_polution(Vector2(position.x,position.y+24))
	else:
		if not pollution_manager.is_polluted(Vector2(position.x,position.y-24)):
			pollution_manager.add_polution(Vector2(position.x,position.y-24))
